...
default_name="openclaw_diag_$(date +%Y%m%d_%H%M%S).log"
    ...
    # 生成诊断日志
    {
        ...
        if [ "$INSTALL_MODE" = "full" ]; then
            ...
            echo "[ config-audit.jsonl ]"
            docker exec openclaw-main tail -n 200 /home/node/.openclaw/logs/config-audit.jsonl 2>/dev/null || echo "  (空)"
            echo "[ gateway.log ]"
            docker exec openclaw-main tail -n 500 /home/node/.openclaw/logs/gateway.log 2>/dev/null || echo "  (空)"
        else
            echo "--- Node.js 信息 ---"
            ...
            echo "--- OpenClaw 业务流与对话审计追踪 ---"
            echo "[ gateway.err.log ]"
            tail -n 200 "$HOME/.openclaw/logs/gateway.err.log" 2>/dev/null || echo "  (空)"
            echo "[ config-audit.jsonl ]"
            tail -n 200 "$HOME/.openclaw/logs/config-audit.jsonl" 2>/dev/null || echo "  (空)"
            echo "[ gateway.log ]"
            tail -n 500 "$HOME/.openclaw/logs/gateway.log" 2>/dev/null || echo "  (空)"
        fi
        echo ""
        echo "--- 安装日志 ---"
        if [ -d "$LOG_DIR" ]; then
